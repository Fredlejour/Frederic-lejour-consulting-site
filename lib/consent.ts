 ...
 * le domaine : il est conservé lors de la navigation entre `/fr`, `/de` et
 * `/en`. Aucun `localStorage` ni `sessionStorage` n’est utilisé.
 *
 * Si `NEXT_PUBLIC_GA_MEASUREMENT_ID` est absent ou vide, tout le dispositif
 * est inerte : aucune bannière, aucun bouton de préférences, aucun cookie.
 */
 ...
 *
 * Le layout racine est porté par le segment dynamique `[locale]` : passer de
 * `/de` à `/en` démonte puis remonte tout le sous-arbre, donc `Analytics` et
 * ses `ref`. Un garde local au composant serait détruit à chaque changement
 * de langue et laisserait republier la configuration.
 *
 * Le module, lui, n’est évalué qu’une fois par document navigateur : la garde
 * survit à tous les remontages, et un véritable rechargement la réinitialise
 * naturellement, sans cookie ni stockage.
 */
...
function cookieDomains(): (string | null)[] {
  ...
  if (hostname === 'lejourconsulting.com' || hostname.endsWith('.lejourconsulting.com')) {
    domains.push('.lejourconsulting.com');
  }
